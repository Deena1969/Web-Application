...
exports.checkoutBookSlot = async (req, res) => {
  ...
  if (error) {
    if (
      error.details[0].path[0] === "vehicleNo" &&
      error.details[0].type === "string.pattern.base"
    ) {
      return res
        .status(400)
        .json({ msg: "Please enter a valid Vehicle Number" });
    }
  }
  ...
  try {
    ...
    if (!user.profilePic) {
      return res
        .status(400)
        .json({ msg: "Please Upload a profile photo first for verification" });
    }
    ...
    if (futureBookedParkingSlots.length > 0) {
      return res.status(400).json({
        msg: `You have already have booked a slot for a ${vehicleType}`,
      });
    }
    ...
    if (vehicleBookedSlots.length > 0) {
      return res.status(400).json({
        msg: `This vehicle Number ${vehicleNo} already has an active slot booked`,
      });
    }
    ...
    if (type === "private") {
      ...
      console.log(
        storebookingStart,
        storebookingEnd,
        mongoose.Types.ObjectId(slotId),
        mongoose.Types.ObjectId(lotId),
        req.userId,
        vehicleType,
        vehicleNo,
        cancellable,
      ...
      console.log({ storebookingStart, storebookingEnd });
      const bookedSlot = await BookedTimeSlot.create({
        startTime: storebookingStart,
        endTime: storebookingEnd,
        parkingSlot: mongoose.Types.ObjectId(slotId),
      ...
      if (parkingLot.type === "private") {
        ...
        const html = `
                Dear ${parkingLot.ownerName},
                    New Booking At Your Parking Lot ${parkingLot.name}, for a ${
          bookedTimeSlot.vehicleType
        } with number ${bookedTimeSlot.vehicleNo} between ${dayjs(
          bookedTimeSlot.startTime
        ).format("DD MMM hh:00 A")} and ${dayjs(bookedTimeSlot.endTime).format(
          "DD MMM hh:00 A"
        )}. 
...
exports.refundPaymentVerification = async (req, res) => {
  ...
  try {
    ...
    if (expectedSign === razorpay_signature) {
      ...
      const bookedTimeSlot = await BookedTimeSlot.findByIdAndUpdate(
        req.query.slotID,
        { refunded: true, refundDetails: paymentDetails },
        { new: true }
      );
      const parkingLot = await ParkingLot.findById(bookedTimeSlot.parkingLot, {
        ownerName: 1,
      ...
      const html = `
            Dear ${req.query.userName},
                    Refund for Your Booking Cancellation, at Parking Lot ${
                      parkingLot.name
                    }, for a ${bookedTimeSlot.vehicleType} with number ${
        bookedTimeSlot.vehicleNo
      } between ${dayjs(bookedTimeSlot.startTime).format(
        "DD MMM hh:00 A"
      )} and ${dayjs(bookedTimeSlot.endTime).format("DD MMM hh:00 A")}. 
                    The charges for this parking you booked ${
                      req.query.charges
                    }, ${
        bookedTimeSlot.adminCancelled
          ? `100% is refunded`
          : `70% i.e.${(req.query.charges * 0.7).toFixed(2)} is refunded`
      } to your account
                From,
                Smart Parking Team
            `;
